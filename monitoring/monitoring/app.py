...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:postgres@localhost/database_monitoring'
...
def fetch_server_details(ip_address, username, password, platform):
    ...
    try:
        if platform == 'Windows':
            ...
            details['operating_system'] = stdout.split('Caption')[1].split('\n')[0].strip() if 'Caption' in stdout else 'Unknown'
            total_ram = stdout.split('TotalVisibleMemorySize')[1].split('\n')[0].strip() if 'TotalVisibleMemorySize' in stdout else '0'
            details['total_ram_gb'] = int(total_ram) // 1024 // 1024 if total_ram and total_ram.isdigit() else 0
            free_disk = stdout.split('FreeSpace')[1].split('\n')[0].strip() if 'FreeSpace' in stdout else '0'
            details['free_disk_gb'] = int(free_disk) // 1024 // 1024 // 1024 if free_disk and free_disk.isdigit() else 0
            total_disk = stdout.split('Size')[1].split('\n')[0].strip() if 'Size' in stdout else '0'
            details['total_disk_gb'] = int(total_disk) // 1024 // 1024 // 1024 if total_disk and total_disk.isdigit() else 0
            cpu_count = stdout.split('NumberOfCores')[1].split('\n')[0].strip() if 'NumberOfCores' in stdout else '0'
            details['cpu_count'] = int(cpu_count) if cpu_count and cpu_count.isdigit() else 0
            details['boot_time'] = stdout.split('LastBootUpTime')[1].split('\n')[0].strip() if 'LastBootUpTime' in stdout else 'Unknown'
            details['status'] = True
        else:
            ...
            details['total_disk_gb'] = int(total_disk) if total_disk and total_disk.replace('.', '').isdigit() else 0
            stdin, stdout, stderr = ssh.exec_command('df -h / | tail -1 | awk \'{print $4}\'')
            free_disk = stdout.read().decode().strip().replace('G', '')
            details['free_disk_gb'] = int(free_disk) if free_disk and free_disk.replace('.', '').isdigit() else 0
...
def dashboard():
    ...
    online_servers = len([s for s in servers if s.status])
    offline_servers = total_servers - online_servers
    # Mock data for recent activities (you can replace with actual data)
    recent_activities = [
        {"user": "Jack", "time": "54 Min Ago"},
        {"user": "Amir", "time": "3 Hr Ago"},
        {"user": "Ember", "time": "6 Hr Ago"}
    ]
    # Mock data for recent orders (you can replace with actual data)
    recent_orders = [
        {"course_name": "JavaScript Tutorial", "course_number": "85743", "payment": "Due", "status": "Pending"},
        {"course_name": "CSS FULL COURSE", "course_number": "97245", "payment": "Refund", "status": "Declined"},
        {"course_name": "FLEX-BOX TUTORIAL", "course_number": "36452", "payment": "Paid", "status": "Active"}
    ]
    return render_template('dashboard.html', servers=servers, total_servers=total_servers,
                          online_servers=online_servers, offline_servers=offline_servers,
                          recent_activities=recent_activities, recent_orders=recent_orders)

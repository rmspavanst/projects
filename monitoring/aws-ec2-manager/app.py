...
def api_instances():
    ...
    for instance in db_instances:
        ...
        print(f"Launch Time (UTC): {launch_time}")
        print(f"Auto-Terminate Time (hours): {auto_terminate_time}")    
        # Calculate expiration time (subtract 1 second to start at HH:59:59)
        expiration_time = launch_time + timedelta(hours=float(auto_terminate_time)) - timedelta(seconds=1)
        print(f"Expiration Time (UTC): {expiration_time}")
        # Calculate remaining time
        remaining_time = max(expiration_time - datetime.now(pytz.utc), timedelta(0))
        print(f"Remaining Time (timedelta): {remaining_time}")
...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            # Validate auto-terminate time (in hours, min 1, max 24)
            if not auto_terminate_time.isdigit() or not (1 <= int(auto_terminate_time) <= 24):
                flash("Invalid auto-terminate time. Must be between 1 and 24 hours.", "error")
                return render_template('index.html', instances=instances, form_data=form_data)

            auto_terminate_time_hours = int(auto_terminate_time)  # Directly use hours
...
def terminate_expired_instances():
    ...
    for instance in db_instances:
        ...
        if launch_time.tzinfo is None:
            launch_time = launch_time.replace(tzinfo=pytz.utc)
        expiration_time = launch_time + timedelta(hours=float(auto_terminate_time)) - timedelta(seconds=1)  # Subtract 1 second
